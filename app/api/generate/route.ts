...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = [
      ...
      `${body.dietPreference === "Blueprint" ? `- Focus on plant-based, nutrient-dense foods` : ""}`,
      `${body.dietPreference === "Blueprint" ? `- Include black lentils, broccoli, cauliflower, shiitake mushrooms` : ""}`,
      `${body.dietPreference === "Blueprint" ? `- Use healthy fats like extra virgin olive oil, hemp seeds` : ""}`,
      `${body.dietPreference === "Blueprint" ? `- Incorporate berries, nuts, and seeds` : ""}`,
